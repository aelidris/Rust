...
impl Tracker {
    ...
    pub fn set_value(&self, value: &Rc<usize>) {
        ...
        if count > self.max {
            self.messages.borrow_mut().push("Error: You can't go over your quota!".to_owned());
            return;
        }
        ...
        if percentage_of_max >= 70 {
            self.messages
                .borrow_mut()
                .push(
                    format!("Warning: You have used up over {percentage_of_max}% of your quota!")
                );
        }
    }
    ...
    pub fn peek(&self, value: &Rc<usize>) {
        ...
        self.messages
            .borrow_mut()
            .push(format!("Info: This value would use {percentage_of_max}% of your quota"));
    }
}
